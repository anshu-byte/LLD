...
class FoodDeliveryServiceDemo:
    ...
    def run():
        ...
        agent1 = DeliveryPartner("D001", "Charlie Brown", "555-8765")
        agent2 = DeliveryPartner("D002", "Daisy Duck", "555-4321")
        service.register_delivery_partner(agent1)
        service.register_delivery_partner(agent2)
        ...
        # Check order details
        print(
            f"Order details: {order.id}, status: {order.status}, items: {[(item.item.name, item.quantity) for item in order._items]}"
        )
